...
import { useAddPostMutation, useGetAllPostsQuery, useGetUserPostsQuery } from '../../store/postServices/memoriesApi'
...
const MainContainer = () => {
    ...
    const [socketPosts, setSocketPosts] = useState(null)

    const { currentUser } = useAppSelector((state) => state.users)
    const { data: posts, refetch: useFetchGetPosts } = useGetUserPostsQuery(username)
    ...
    return (
        <Box className={classes.mainWrapper} sx={{ pt: '16px', flexWrap: 'none' }}>
            ...
            <Grid className={classes.flexWrapper} sx={{ pt: '16px' }} container direction="row">
                <CategoryPaper width="60%" activeCat={activeCat} setActiveCat={setActiveCat} catToPick={catToPick} images={images} />
            ...
            <Grid sx={{ marginTop: '16px' }}>
                {isOwner && (
                    <AccountPostsPaper handleSubmitForm={handleSubmitForm} addPostData={addPostData} setAddPostData={setAddPostData} />
                )}
                <Grid>
                    {posts !== undefined ? (
                        posts?.map((post: any) => <PostItem key={post._id} useFetchGetPosts={useFetchGetPosts} post={post} />)
                    ) : (
